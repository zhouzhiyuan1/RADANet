...
def ana(nme_list):
    ...
    #     #
    save_np_mat(nme_all, './nme_mfirrn.mat')
...
#     # save_np_mat(nme_all, './nme2000_DAMD_all.mat')
#     #
#     save_np_mat(nme_all, './nme_mobilenet_11.mat')
#     mean_nme_1 = np.mean(nme_1) * 100
#     mean_nme_2 = np.mean(nme_2) * 100
#     mean_nme_3 = np.mean(nme_3) * 100
#     # mean_nme_all = np.mean(nme_list) * 100
...
#
#         # nme
#         pt_valid = (pts21_gt[0, :] != -1) & (pts21_gt[1, :] != -1)              #&与运算？
#         dis = pts21_est[:, pt_valid] - pts21_gt[:, pt_valid]
#         dis = np.sqrt(np.sum(np.power(dis, 2), 0))              #power：dis的2次方
#         dis = np.mean(dis)
#         nme = dis / llength
#         nme_list.append(nme)
#
#     nme_list = np.array(nme_list, dtype=np.float32)
#     return nme_list             #nme：均一化平方差
#
#
# def main():
#     pass
