...
def main(args):
    ...
    # #checkpoint_fp = '/data1/zhouzhiyuan/DFA/DAMDNet.pth.tar'
    checkpoint_fp = '/data1/lab105/zhouzhiyuan/3ddfa/replay/phase1_wpdc_checkpoint_epoch_37.pth.tar'
    arch = ''
    ...
    for img_fp in args.files:
        ...
        for rect in rects:
            ...
            if args.dump_roi_box:
                wfp = '{}_{}_our.roibox'.format(img_fp.replace(suffix, ''), ind)
                np.savetxt(wfp, roi_box, fmt='%.3f')
                print('Save roi box to {}'.format(wfp))
            if args.dump_paf:
                wfp_paf = '{}_{}_our_paf.jpg'.format(img_fp.replace(suffix, ''), ind)
                wfp_crop = '{}_{}_our_crop.jpg'.format(img_fp.replace(suffix, ''), ind)
            ...
            if args.dump_obj:
                wfp = '{}_{}_our.obj'.format(img_fp.replace(suffix, ''), ind)
        ...
        if args.dump_pose:
            ...
            wfp = img_fp.replace(suffix, 'our_pose.jpg')
            cv2.imwrite(wfp, img_pose)
            print('Dump to {}'.format(wfp))
        if args.dump_depth:
            wfp = img_fp.replace(suffix, 'our_depth.png')
            ...
            print('Dump to {}'.format(wfp))
        if args.dump_pncc:
            wfp = img_fp.replace(suffix, 'our_pncc.png')
