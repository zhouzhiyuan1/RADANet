...
def save_np_mat(data, path):
    data_np = np.array(data, dtype=np.float32)
    io.savemat(path, {'name': data_np})
def ana(nme_list):
    ...
    nme_all = np.concatenate((nme_1, nme_2, nme_3))
    #     # nme_all=np.concatenate((nme_1,nme_2,nme_3))
    #     # save_np_mat(nme_all, './nme2000_DAMD_all.mat')
    #     #

    mean_nme_1 = np.mean(nme_1) * 100
    mean_nme_2 = np.mean(nme_2) * 100
    mean_nme_3 = np.mean(nme_3) * 100
    save_np_mat(mean_nme_2, './3dnme_damd_m.mat')
    save_np_mat(mean_nme_3, './3dnme_damd_l.mat')
...
def calc_nme(pts68_fit_all, option='ori'):
    ...
    for i in range(len(roi_boxs)):
        ...
        minpx, maxpx = np.min(pts68_fit[0, :]), np.max(pts68_fit[0, :])
        miny, maxy = np.min(pts68_gt[1, :]), np.max(pts68_gt[1, :])
        minpy, maxpy = np.min(pts68_fit[1, :]), np.max(pts68_fit[1, :])
        minz, maxz = np.min(pts68_gt[2, :]), np.max(pts68_gt[2, :])
        minpz, maxpz = np.min(pts68_fit[2, :]), np.max(pts68_fit[2, :])

        pts68_fit[0, :] = (pts68_fit[0, :] - minpx) / (maxpx - minpx)
        pts68_fit[0, :] = pts68_fit[0, :] * (maxx - minx) + minx
        pts68_fit[1, :] = (pts68_fit[1, :] - minpy) / (maxpy - minpy)
        pts68_fit[1, :] = pts68_fit[1, :] * (maxy - miny) + miny
        pts68_fit[2, :] = (pts68_fit[2, :] - minpz) / (maxpz - minpz)
        pts68_fit[2, :] = pts68_fit[2, :] * (maxz - minz) + minz

        llength = pow((maxx - minx) * (maxy - miny) * (maxz - minz), 1 / 3.)

        #
        dis = pts68_fit - pts68_gt[:3, :]
